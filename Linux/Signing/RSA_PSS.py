...
def generate_keys():
    ...
    with open("/content/CryptoBenchmark/Linux/Signing/private.pem", "wb") as f:
        f.write(private_key)
    
    # Exportación de la clave pública al archivo public.pem. Abre el archivo public.pem, y lo crea si es que no existe.
    public_key = key.publickey().export_key()
    with open("/content/CryptoBenchmark/Linux/Signing/public.pem", "wb") as f:
        f.write(public_key)
...
def signing(file):
    ...
    privatekey = RSA.import_key(open('/content/CryptoBenchmark/Linux/Signing/private.pem').read(), frase)
    ...
    publickey = RSA.import_key(open('/content/CryptoBenchmark/Linux/Signing/public.pem').read())
    hash = SHA256.new(archivo)
